        ...
        <tbody>
            ...
            {#each rowData as row, index}
                <tr>
                    <td>{row.name}</td>
                    <td>
                        <Select optionsCount={21} onChange={(value) => handleExerciseVariables(value, 'sets', row.eid)} />
                    </td>
                    <td>
                        <Select optionsCount={21} onChange={(value) => handleExerciseVariables(value, 'reps', row.eid)} />
                    </td>
                    <td>
                        <input type="number" min="0" max="1000" name="weight" on:input={(event) => handleExerciseVariables(event.target.value, 'weight', row.eid)} />
                    </td>
                    <td>
