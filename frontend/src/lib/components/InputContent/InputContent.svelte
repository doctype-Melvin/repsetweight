<script>
    ...
    const editWorkout = (workoutID, content, type) => {
        ...
        if (type === 'description') {
            ...
            console.log($userTemplateData.workouts[index].description)
        } else {
            userTemplateData.update(template => {
                template.workouts[index].name = content;
                return template;
            })
            console.log($userTemplateData.workouts[index].name)
        }
...
<div>
    {#if $isInput && !type}
        <input type='text' value={content} autofocus on:input={handleInput} on:blur={() => editWorkout(wid, $newContent, type)} >
    {:else if $isInput && type}
        <textarea value={content} rows="3" cols="40" autofocus on:input={handleInput} on:blur={() => editWorkout(wid, $newContent, type)}></textarea>
    {:else}
